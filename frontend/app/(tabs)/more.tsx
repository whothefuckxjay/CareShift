...
import { nurseProfile } from "@/data/mockData";
        ...
        <View style={[styles.page, { maxWidth: contentMaxWidth, alignSelf: "center", width: "100%" }]}>
          <Pressable onPress={() => router.push("/profile")}>
            <Card style={styles.profileCard}>
              <Avatar uri={user?.avatar ?? nurseProfile.avatar} name={user?.name ?? nurseProfile.name} size={56} />
              <View style={{ marginLeft: 14, flex: 1 }}>
                <Text style={[styles.profileName, { color: colors.text }]}>{user?.name ?? nurseProfile.name}</Text>
                <Text style={[styles.profileRole, { color: colors.textSecondary }]}>
                  {user?.role ?? nurseProfile.role} · {nurseProfile.department}
                </Text>
              </View>
